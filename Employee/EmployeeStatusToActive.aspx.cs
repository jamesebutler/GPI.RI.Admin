...
{
    public partial class EmployeeStatusToActive : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            LabelSetStatus.Text = "Set Employee to Active for " + mydefaultsiteName;
            ...
            if (!IsPostBack)
            {
                // LoadActiveEmployees();
                LoadEmployees();
                LoadEmployeesforGrid(mydefaultsiteID, "Y");
        ...
        protected void LoadActiveEmployees()
        {
            ...
            string Sql1 = "Select username,  INITCAP(lastname) || ', ' || INITCAP(firstname) fullname  from refemployee where 1=1 and inactive_flag = 'Y' and  siteid = '" + userSite + "' ORDER BY lastname,FIRSTNAME";
        ...
        {
            string employeeSelected = DropDownEmployees.SelectedValue;
            string userplantcode = Session["PlantCode"].ToString();
            string returnvalue = usermodel.UpdateEmployeeStatusToActive(
                employeeSelected,
            ...
            if (returnvalue == "0")
            {
                ...
                //go add task tracker notifications
                Boolean myreturn = AddTaskTrackerNotifications(employeeSelected, mySessionUserName);
